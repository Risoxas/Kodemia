...
const getContinent = (region) => countriesArray.filter(continent => continent.region === region)

/***************************************** 2 ****************************************/

let totalPopulation = countriesArray.reduce((accum, current) => accum + current.population, 0)

/***************************************** 3 ****************************************/


const getPercentage = (pais) => (countriesArray.find(country => country.name === pais).population) / totalPopulation

/***************************************** 4 ****************************************/
const getName = (alpha) => {
    let foundCountry = countriesArray.filter(country => country.alpha2Code === alpha)
    return foundCountry ? foundCountry.map(pais => `EL país es ${pais.name}, su población es de ${pais.population} y su area total es de ${pais.area} km^2`) : "el pais no se encuentra en al lista"]
}

/***************************************** 5 ****************************************/

const getBiggerArea = (alpha1,alpha2) => {
    let 
}

/***************************************** 6 ****************************************/
const getMaxPopulation = (arr) => arr.reduce((accum,current) => {
    return accum.population > current.population ? accum.population : current.population
},accum[0])
const getMinPopulation = (arr) => arr.reduce((accum,current) => {
    return accum.population < current.population ? accum.population : current.population
},accum[0])
